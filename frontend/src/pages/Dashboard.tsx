...
export function Dashboard() {
  ...
  // ── Focus popup (shown after clock-in) ────────────────────────────────────
  ...
  const toggleFocusTask = (title: string) =>
    setFocusSelected(prev => prev.includes(title) ? prev.filter(t => t !== title) : [...prev, title]);

  return (
    <div className="space-y-6">

      {/* ── Focus popup (after clock-in) ── */}
      {showFocusPopup && (
          ...
          <div className="w-full max-w-md rounded-xl p-6 shadow-2xl" style={{ backgroundColor: VS.bg1, border: `1px solid ${VS.border}` }}>
            {!focusDone ? (
              <>
                ...
                <p className="text-[12px] mb-4" style={{ color: VS.text2 }}>Select one or more tasks to focus on</p>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    Let's go! {focusSelected.length > 0 && `(${focusSelected.length})`}
                  </button>
                </div>
              </>
            ) : (
