...
const getMigration = require('./better-auth.Bk0IzLeq.cjs');
...
async function createTelemetry(options, context) {
  ...
  const disableNotice = options.telemetry?.disableNotice || options.telemetry?.enabled === false || options.telemetry?.debug || env.getBooleanEnvVar("BETTER_AUTH_TELEMETRY_DISABLE_NOTICE", false);
